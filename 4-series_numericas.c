...
scanf("%d", &serie1);
   for(contador=0;contador<=serie1;contador++)
    ...
    printf ("\n\nIntroduzca el numero hasta el que quiere llegar en la serie 4: \n\n  ");
    scanf("%d", &serie1);
    for (int contador=1; contador<=serie1; contador++)
    {
        printf("  %d  %d",Npositivo, Nnegativo);
        Npositivo++;
        Nnegativo--;
        contador++;
}
printf("\nIngrese el numero de elementos de las serie 2\n");//error de visual studio
scanf("%d", &serie1);
for (contador=0;contador < serie1; contador++)
{
        El_nuemro_del_usuario=pow(contador,3);
        El_nuemro_del_usuario=El_nuemro_del_usuario+pow(contador,2);
        printf("%d ",El_nuemro_del_usuario+2);
    }
}
